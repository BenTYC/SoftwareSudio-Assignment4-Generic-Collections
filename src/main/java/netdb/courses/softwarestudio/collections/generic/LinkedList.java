...
public class LinkedList<E> implements List<E>, Deque<E> {
	...
	private static class Entry<E> {
		...
		Entry(E e, Entry<E> next, Entry<E> previous) {
			element = e;
			this.next = next;
	...
	private E remove(Entry<E> entry) throws UnderflowException {
		if(size == 0)
	...
	public void remove(E e) {
		if(e == null) {
			for(Entry<E> entry = header.next; entry != header;) {
				Entry<E> tmp = entry.next;
				if(entry.element == null) {
					remove(entry);
				}
				entry = tmp;
